...
class EGDExchange():
    ...
    def addNewTag(self, id, name, tagtype, offsetbyte, offsetbit, description=''):
        tag = EGDTag(id, name, tagtype, self.exchangenumber,
                     offsetbyte, offsetbit, description)
    ...
    def setLastMessage(self, egdmessage):
        ...
        self.lasttimestamp = datetime.datetime.fromtimestamp(
            egdmessage.header.timestampsecs + (egdmessage.header.timestampnanosecs / 10e9))
        self.lastnow = datetime.datetime.now()

    def dump(self):
        pre = "\t{} ({} ms) - {} - {}".format(self.exchangenumber,
                                              self.period, self.codec[:10]+"...", struct.calcsize(self.codec))
...
class EGDPayload(object):
    ...
    def __init__(self, payloaddata, codec):
        ...
        if EGDPayload.MAX_PAYLOAD_DATA_LEN > len(payloaddata):
            payloaddata += chr(0) * \
                (EGDPayload.MAX_PAYLOAD_DATA_LEN - len(payloaddata))
